...
more_data <- dir("data/reports", full.names = T) %>%
    ...
    mutate(date_produced = str_remove_all(path, "data/reports/FacebookAdLibraryReport_|_TR_yesterday_advertisers\\.csv")) %>%
  ...
  mutate(party1 = case_when(
    ...
    str_detect(page_name, "\\bAKP|Recep Tayyip Erdoğan\\b") ~ "AKP",
    str_detect(page_name, "\\bMHP\\b") ~ "MHP",
    T ~ NA_character_
  ))%>%
