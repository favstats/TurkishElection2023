---
title: "2023 Turkish general election"
...
the_order <- contested_dat %>% 
  filter(party == "AKP") %>%   arrange(desc(perc)) %>% 
  pull(value) %>% 
  unique()
...
ready_mhp %>% 
  ...
  # geom_sf_label(data = prov, aes(label = name)) +
    ...
    guides(fill = guide_colourbar(title = "Spend (₺)", barwidth = 15, barheight = 0.5)) +
  coord_sf() +
  geom_sf_label(data = ready_mhp %>% distinct(total_spend, .keep_all = T) %>% drop_na(total_spend),
                aes(label = paste0("₺",scales::comma_format()(round(total_spend))))) +
  labs(title = "MHP Spending on Ads targeting Provinces") +
  theme(plot.title = element_text(hjust = 0.5))
```
...
ready_apk %>% 
  ...
  geom_sf_label(data = ready_apk %>% distinct(total_spend, .keep_all = T) %>% drop_na(total_spend),
                aes(label = paste0("₺",scales::comma_format()(round(total_spend))))) +
  labs(title = "AKP Spending on Ads excluding Provinces") +
  theme(plot.title = element_text(hjust = 0.5))
```
...
ready_mhp %>% 
  ...
  geom_sf_label(data = ready_mhp %>% distinct(total_spend, .keep_all = T) %>% drop_na(total_spend),
                aes(label = paste0("₺",scales::comma_format()(round(total_spend))))) +
  labs(title = "MHP Spending on Ads excluding Provinces") +
  theme(plot.title = element_text(hjust = 0.5))
```
...
ready_chp %>% 
  ...
  geom_sf_label(data = ready_chp %>% distinct(total_spend, .keep_all = T) %>% drop_na(total_spend),
                aes(label = paste0("₺",scales::comma_format()(round(total_spend))))) +
  labs(title = "CHP Spending on Ads excluding Provinces") +
  theme(plot.title = element_text(hjust = 0.5))
```
