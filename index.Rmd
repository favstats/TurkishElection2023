---
title: "2023 Turkish presidential election (Runoff)"
...
color_dat <- tibble(
  colors = c("#ed1c24", "#ff9901", "#870000", "#3db5e6", "#388537", "#0069a7",
             "#373736", "#0d5da6", "#b10a11", "#ff2700"),
  party = c("CHP", "AKP", "MHP", "İYİ", "YSP", "DEVA",
            "ATA Alliance", "Memleket Partisi", "YRP", "BBP"))
...
election_dat30 <- readRDS("data/election_dat30.rds") %>% 
  ...
  mutate(party = case_when(
    str_detect(page_name, "\\bDEVA\\b|\\bDeva\\b|Ali Babacan")  ~ "DEVA",
    str_detect(page_name, "Yeniden Refah Partisi|Yeniden REFAH Partisi|Şükrü Yılmaz")  ~ "YRP",
    str_detect(party, "ATA")  ~ "ATA Alliance",
    str_detect(party, "Mem")  ~ "Memleket Partisi",

    str_detect(page_name, "ıdvan Eşin|Osman Seçgin")  ~ "BBP",
    T ~ party  ))  %>% 
    filter(!(party %in% c("Bağ", "Diğ"))) %>% 
  drop_na(party)
# , "Mem", "ATA"
...
**If you are looking for the targeting data for Round 1: [click here](https://refined-github-html-preview.kidonng.workers.dev/favstats/TurkishElection2023/raw/ce6281fe74b8f5a3f99c576c31bd95758cf80dec/index.html).**
...
#     labs(y = "Daily Spend in Turkish Lira", x = "Day")
...
add_ribbons("CHP", "#ed1c24") %>%
add_ribbons("AKP", "#ff9901") %>%
add_ribbons("MHP", "#870000") %>%
# add_ribbons("İYİ", "#3db5e6") %>%
# add_ribbons("YSP", "#388537") %>%
add_ribbons("DEVA", "#0069a7") %>%
# add_ribbons("ATA Alliance", "#373736")%>%
add_ribbons("Memleket Partisi", "#0d5da6")#%>%
# add_ribbons("YRP", "#b10a11") #%>%
# add_ribbons("BBP", "#ff2700")
...
library(tidytext)
 interest_targeting30 %>% 
  ...
  scale_fill_parties() +
    ...
    scale_x_reordered()  +
  ggthemes::theme_hc() +
  labs(x = "Exclusion criteria\n", y = "Budget spent on targeting method (in Turkish Lira)", #title = "Top Excluded Audiences by Party", subtitle = "On Meta you can exclude audiences that you don't want to see your ads\n", 
...
contested_dat7 %>% 
    ...
    scale_fill_parties() +
  ggthemes::theme_hc() +
  labs(x = "Targeting criteria", y = "Budget spent on targeting method (in Turkish Lira)", #title = "Contested targeted interests (US midterms 2022)", subtitle = " ", 
...
<!--   filter(party == "CHP") %>%   arrange(desc(perc)) %>%  -->
<!--   pull(value) %>%  -->
<!--   unique() -->
...
region_dat <- election_dat7 %>%
  ...
  ungroup() %>%
  rename(region = value) %>%
  mutate(region = str_remove_all(region, ", Turkey")) 
...
```{r}
geo <- sf::st_read("https://raw.githubusercontent.com/alpers/Turkey-Maps-GeoJSON/master/tr-cities-kktc-airports.json", quiet = T)
```


#### Provinces (Maps) {.tabset .tabset-fade .tabset-pills}

##### AKP


```{r, eval = T, fig.width=15, fig.height=10, dpi=900}
# election_dat30 %>% 
#   filter(is_exclusion) %>% 
#   # filter(party == "PvdA") %>% 
#   arrange(desc(total_spend_pct))
...
#   
# election_dat30 %>% 
#   filter(value == "Elon Musk")

# geojs <- geojson::("https://raw.githubusercontent.com/alpers/Turkey-Maps-GeoJSON/master/tr-cities-kktc-airports.json")
...
AKP_dat <- election_dat7 %>% 
  filter(type == "location") %>% 
  filter(location_type == "regions") %>% 
  mutate(value = str_remove_all(value, " Province, Turkey") %>% str_trim %>% str_squish) %>%
  rename(name = value) %>% 
  filter(party == "AKP") %>% 
  filter(!is_exclusion) %>%
...
ready_apk <- AKP_dat %>% 
  filter(str_detect(name, "Havalimanı", negate = T)) %>% 
  filter(number != 82) 
...
ready_apk %>% 
  ...
  # geom_sf_label(data = prov, aes(label = name)) +
    ...
    guides(fill = guide_colourbar(title = "Spend (₺)", barwidth = 15, barheight = 0.5)) +
  coord_sf() +
  geom_sf_label(data = ready_apk %>% distinct(total_spend, .keep_all = T) %>% drop_na(total_spend),
                aes(label = paste0("₺",scales::comma_format()(round(total_spend))))) +
  labs(title = "AKP Spending on Ads targeting Provinces") +
...
ready_chp %>% 
  ...
  # geom_sf_label(data = prov, aes(label = name)) +
    ...
    guides(fill = guide_colourbar(title = "Spend (₺)", barwidth = 15, barheight = 0.5)) +
  coord_sf() +
  geom_sf_label(data = ready_chp %>% distinct(total_spend, .keep_all = T) %>% drop_na(total_spend),
                aes(label = paste0("₺",scales::comma_format()(round(total_spend))))) +
  labs(title = "CHP Spending on Ads targeting Provinces") +
  theme(plot.title = element_text(hjust = 0.5))
```
...
### `r last30days_string` (Last 7 days) {.tabset .tabset-fade .tabset-pills}
